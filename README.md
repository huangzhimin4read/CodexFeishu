...
- **Bidirectional message mirroring:** Codex user text and images are mirrored to the matching topic. Referenced project-local files up to 20 MiB are uploaded as native downloadable Feishu/Lark file messages instead of path-only labels. The official `lark-cli` may send text as whichever authorized user profile is currently ready; startup does not compare that profile with a configured Open ID. Provider-message ancestry and durable `thread + turn + item` identity prevent the resulting callback and Feishu-origin item from looping or appearing twice.
- **Readable mobile output:** Codex process and final output is delivered through native Feishu/Lark `lark_md` message cards, preserving supported Markdown such as emphasis, lists, code blocks, and labeled web links. Local paths remain hidden. A project-local Markdown image embedded between text paragraphs is uploaded first and then inserted at its original position in the same card.
- **Native file delivery:** PDF, STEP/STP, IGES/IGS, DXF/DWG/STL/FCStd, common Office/text data, and common archive formats are captured only from the bound project root. Feishu cards cannot contain binary attachments, so each file is sent immediately after its matching Markdown body in the same topic and turn. Upload completion is persisted before the visible file message is attempted.
...
- **Remote inputs:** independently gated text, image, and file input. The recommended `cli` mode uses only Codex CLI. An uplink that has not reached a confirmed Codex user turn within 60 seconds is discarded, will not be retried again, and receives a visible Feishu/Lark timeout notice asking the user to resend. This mode never manipulates the Codex Desktop composer. Files are bounded, hashed, stored under the selected project's inbox, and never auto-executed or auto-extracted.
- **Truthful submission status:** Feishu/Lark reports `submitted` only after the exact Codex user turn is confirmed. Before the 60-second deadline, an unavailable writer is reported as pending or unconfirmed; after the deadline, the bridge reports that the uplink was discarded instead of claiming delivery. Feishu's hollow read-status circle is native client UI and cannot be cleared by the bridge or the normal Feishu API.
