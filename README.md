...
- **Readable mobile output:** process/final messages, project-local Markdown images, visible Codex image outputs, file-citation labels, and link destinations hidden from provider-visible text. A project-local image embedded between text paragraphs is uploaded first and then delivered in one ordered Feishu/Lark rich-text post, so the image stays at its original position instead of becoming a separate message.
