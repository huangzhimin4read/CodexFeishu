...
- **双向消息镜像：**Codex 中的用户文本和图片同步到对应飞书话题；被正文引用、位于当前项目内且不超过 20 MiB 的文件，会上传成可直接下载的飞书/Lark 原生附件，不再只有无路径标签。官方 `lark-cli` 可以使用当前已就绪的任意授权用户配置发送文本，启动时不再把该账号与配置中的 Open ID 比较。桥接器通过消息 ancestry 和持久 `thread + turn + item` 身份阻止回调和飞书来源 item 循环或重复显示。
- **适合手机阅读：**Codex 过程消息和 final 使用飞书/Lark 原生 `lark_md` 消息卡片发送，保留飞书支持的强调、列表、代码块和带标签网页链接等 Markdown 效果；本地路径仍不暴露。项目内 Markdown 图片会先完成上传，再插入同一卡片中的原位置。
- **原生文件下发：**支持 PDF、STEP/STP、IGES/IGS、DXF/DWG/STL/FCStd、常用 Office/文本数据和压缩包格式。飞书卡片不能承载二进制附件，因此文件会紧跟对应 Markdown 正文，在同一话题和同一轮次中按引用顺序发送；系统先持久化上传结果，再尝试发送可见附件消息。
