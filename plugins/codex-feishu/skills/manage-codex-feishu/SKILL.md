...
- Render Codex process and final text with native Feishu/Lark `lark_md` message-card elements while keeping local paths and file-citation destinations hidden. Web links may remain clickable behind their visible labels. Upload allowed project-local files of at most 20 MiB as native file messages immediately after the matching body, in citation order; support common PDF, CAD, Office, text-data, and archive suffixes. When a project-local Markdown image is embedded between visible text, upload every image key first and send one ordered Markdown card; never expose a partial card or move the image to a separate trailing message.
