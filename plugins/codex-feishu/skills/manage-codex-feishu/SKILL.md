...
- `delivery = "cli"` persists through `codex exec resume` and attaches local images with `--image`. Codex allows one writer per task: when another writer owns the task, retain the ingress in the durable queue and retry with CLI later. Never manipulate the Codex Desktop composer while `delivery = "cli"`. `delivery = "desktop_relay"` uses one internal relay task identified only by `desktop_relay_thread_id`; its name may change. Prefill through `codex://threads/<id>?prompt=...`, identify the renderer by the unique prompt, and avoid global keyboard/clipboard input or foreground activation. Confirm the exact relay and target rollout items before acceptance. `app_server` runs directly under the current Windows user and supports `dangerFullAccess` without a separate worker.
