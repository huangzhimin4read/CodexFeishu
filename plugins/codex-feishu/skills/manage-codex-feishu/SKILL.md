...
- Render links and file citations as path-free visible labels. Upload accessible local images/files instead of exposing their paths. When a project-local Markdown image is embedded between visible text, upload every image key first and send one ordered Feishu/Lark rich-text post; never expose a partial post or move the image to a separate trailing message.
